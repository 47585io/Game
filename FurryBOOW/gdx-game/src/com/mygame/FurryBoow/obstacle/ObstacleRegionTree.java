...
public class ObstacleRegionTree implements ObstacleContainer
{
	...
	public ObstacleRegionTree(ObstacleContainer container, int left, int top, int right, int bottom)
	{
		...
		mRectOfObstacle = new IdentityHashMap<>(mObstacleCount);
	...
	private static abstract class RTNode
	{
		...
		protected int insertIndex; //记录刚刚插入的子节点在数组中的下标
		protected int deleteIndex; //记录刚刚删除的子节点在数组中的下标
		...
		/* 叶子节点中刚刚删除了一个条目，如果叶子节点中的剩余条目数太少
		   需要删除这个叶子节点，同时将该结点中剩余的条目重插入到其他结点中
		 ...
		 */
		protected static int[] pickSeeds(Rect[] rects, int count)
		{
