...
public class ObstacleRegionTree implements ObstacleContainer
{
	...
	/* 用指定容器中的指定范围内的物体构建一颗新的区域树，并且裁剪它们的范围 */
	public ObstacleRegionTree(ObstacleContainer container, int left, int top, int right, int bottom)
	{
		//获取容器内指定范围内的物体
		Obstacle[] objs = container.getObstacles(left, top, right, bottom);
		mObstacleCount = objs.length;
		mTree = new RTree();
		if(mObstacleCount == 0){
			return;
		}
		//将物体一个个添加到树中，并且令这些物体的区域不超出裁剪范围
		...
		for(int i = 0; i < mObstacleCount; ++i){
			...
			mTree.insert(rect, objs[i]);
			mRectOfObstacle.put(objs[i], rect);
		}
	}
	...
	public Obstacle[] getObstacles(int left, int top, int right, int bottom)
	{
		...
		//创建数组，将物体拷贝到数组中，回收列表
		Obstacle[] objs = new Obstacle[list.size()];
		list.toArray(objs);
		recyle(list);
		return objs;
	}
	...
	public ObstacleContainer subContainer(int left, int top, int right, int bottom){
		return new ObstacleRegionTree(this, left, top, right, bottom);
	}
	...
	/* 检查边界 */
	private static void checkBounds(int left, int top, int right, int bottom){
		
	}

	/* 获取临时物体容器 */
	private static List obtain()
	{
		synchronized(sCachedBuffer)
		{
			for(int i = sCachedBuffer.length - 1; i >= 0; --i){
				List buffer = sCachedBuffer[i];
				if(buffer != null){
	...
	/* 回收临时物体容器 */
	private static void recyle(List buffer)
	{
		buffer.clear();
		synchronized(sCachedBuffer)
		{
	...
	private IdentityHashMap<Obstacle, Rect> mRectOfObstacle; //每个物体的矩形区域

	private ObstacleBoundsFilter[] mFilters = NO_FILTERS; //过滤器列表
	private static final ObstacleBoundsFilter[] NO_FILTERS = new ObstacleBoundsFilter[0];
	private static final List[] sCachedBuffer = new List[6]; //存储临时物体容器
